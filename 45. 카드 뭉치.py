...
def solution(cards1, cards2, goal):
    ...
    for i in range(len(goal)):
        if len(cards1) != 0 or len(cards2) != 0:
            if len(cards1) != 0 and goal[i] in cards1[0]:
                answer_check.append(cards1[0])
                cards1.pop(0)
            elif len(cards2) != 0 and goal[i] in cards2[0]:
                answer_check.append(cards2[0])
                cards2.pop(0)

        if answer_check == goal:
            answer = "Yes"
        else:
            answer = "No"
